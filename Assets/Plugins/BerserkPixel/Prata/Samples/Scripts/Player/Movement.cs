...
namespace Project.Scripts.Player
{
    public class Movement : MonoBehaviour
    {
        ...
        private void FixedUpdate()
        {
            ...
            {
                float amount = Mathf.Min(Mathf.Abs(rb.linearVelocity.y), Mathf.Abs(slideForce));
                amount *= Mathf.Sign(rb.linearVelocity.y);
                rb.AddForce(Vector2.up * -amount, ForceMode2D.Impulse);

                if (playerInput.Climb)
                {
                    rb.linearVelocity = new Vector2(rb.linearVelocity.x, climbSpeed * moveInput.x);
                }
            }
            ...
            if (rb.linearVelocity.y < 0 && lastGroundedTime <= 0 && !isSliding)
            {
        ...
        private void WallJump(float jumpForceX, float jumpForceY)
        {
            ...
            float momentumForce = rb.linearVelocity.x * Mathf.Sign(jumpForceX);
